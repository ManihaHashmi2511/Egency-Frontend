...
const CaseStudiesTable = () => {
  ...
  const swapOrder = async (indexA, indexB) => {
    ...
    try {
      await Promise.all([
        api.put(`/case-studies/${itemA._id}`, { order: itemB.order }),
        api.put(`/case-studies/${itemB._id}`, { order: itemA.order }),
      ]);
      fetchItems();
    } catch (error) {
  ...
  const handleDelete = async (item) => {
    ...
    try {
      await api.delete(`/case-studies/${item._id}`);
      Swal.fire("Deleted", "Case study removed successfully", "success");
      fetchItems();
    } catch (error) {
  ...
  return (
    <div>
      ...
      {loading ? (
        <div className="text-center text-gray-400 text-base py-14">Loading case studies...</div>
      ) : filtered.length === 0 ? (
        <div className="text-center text-gray-400 text-base py-14">No case studies found</div>
      ) : (
          ...
          {filtered.map((item, index) => (
            ...
            >
              ...
              <div className="p-6">
                ...
                {item.client && <p className="text-gray-500 text-sm mb-2">Client: {item.client}</p>}

                {item.result && (
                  <div className="flex items-center gap-1.5 text-emerald-600 text-sm font-medium mb-4">
                    <MdOutlineTrendingUp className="text-lg" /> {item.result}
                  </div>
                )}

                <div className="flex items-center gap-2">
                  <button
                    onClick={() => navigate("/admin/case-studies/edit", { state: { caseStudy: item } })}
